# Based on https://lightning.ai/docs/pytorch/stable/notebooks/course_UvA-DL/13-contrastive-learning.html
# Modified by Dan Zhao and Fares Abawi
import logging
import os
import argparse
...
class ImageBindTrain(L.LightningModule):
    def __init__(self, lr=5e-4, weight_decay=1e-4, max_epochs=500, batch_size=32, num_workers=4, seed=42, 
                 ...
                 lora=False, lora_horiz=False, lora_rank=4, lora_checkpoint_dir="./.checkpoints/lora",
                 lora_layer_idxs=None, lora_modality_names=None,
                 linear_probing=False, horizontal_scale=True, scaling_factor=2
                 ):
        super().__init__()
        assert not (linear_probing and lora and lora_horiz), \
            "Linear probing is a subset of LoRA training procedure for ImageBind. " \
            "Cannot set both linear_probing=True and lora=True and lora_horiz=True"
            
        self.save_hyperparameters()

        # Load full pretrained ImageBind model
        self.model = imagebind_model.imagebind_huge(pretrained=True)
        if lora or lora_horiz:
...
def parse_args():
    ...
    parser.add_argument("--datasets", type=str, nargs="+", default=["dreambooth"], choices=["dreambooth", "esc"],
    ...
    parser.add_argument("--lora_horiz", action="store_true", help="Use LoRA horizontal")
    parser.add_argument("--lora_horiz_num", type=int, default=2, help="Number of LoRA matrix updates")    
...
if __name__ == "__main__":
    ...
    if "dreambooth" in args.datasets:
        ...
        train_datasets.append(DreamBoothDataset(
            ...
            transform=ContrastiveTransformations(contrast_transforms,
                                                 n_views=1 if args.self_contrast else 1)))
        test_datasets.append(DreamBoothDataset(
            root_dir=os.path.join(args.datasets_dir, "dreambooth", "dataset"), split="test",
            transform=ContrastiveTransformations(contrast_transforms,
                                                 n_views=1 if args.self_contrast else 1)))
    
    elif "esc" in args.datasets:
        from datasets.dreambooth import ESCaudio_FT
        train_datasets.append(DreamBoothDataset(
            root_dir=os.path.join(args.datasets_dir, "esc", "dataset"), split="train",
            transform=ContrastiveTransformations(contrast_transforms,
                                                 n_views=1 if args.self_contrast else 1)))
        test_datasets.append(DreamBoothDataset(
            root_dir=os.path.join(args.datasets_dir, "esc", "dataset"), split="test",
            transform=ContrastiveTransformations(contrast_transforms,
                                                 n_views=1 if args.self_contrast else 1)))
    ...
    ## output of dataloader is (modality batch 1 (dim data, batch size), modality string 1 (dim batch size), batch 2, modality string 2, etc.)
    ...
    lora_layer_idxs = args.lora_layer_idxs if args.lora_layer_idxs else {}
    lora_modality_names = []
    ...
    model = ImageBindTrain(max_epochs=args.max_epochs, batch_size=args.batch_size, lr=args.lr,
                           ...
                           temperature=args.temperature, horizontal_scale=args.lora_horiz, scaling_factor=args.lora_horiz_num,
